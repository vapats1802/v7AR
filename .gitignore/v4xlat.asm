...
N2X DC8 '0123456789ABCDEF'  ; nybble-to-hex digit LUT
PADxp DC32  PADx    ; lit ptr to PADx
PADp  DC32  PAD   ; lit ptr to PAD


padi  ; init PADx <-- #PAD
  ldr x, PADxp  ; ptr to PADx
  ldr w, PADp   ; ptr to PAD
  str w, [x]    ; PADx now points to PAD
  NEXT


i2x$  ; ( -- )  uint-to-ASCIIz hex via PADx, (implicitly) non-destructive
  ldr x, PADxp  ; ptr to PADx
  ldr x, [x]    ; get PADx
  adr k, N2X    ; ptr to LUT
  ...
  ldr w, PADxp  ; ptr to PADx
  str x, [w]    ; PADx now points to next free 'printable' byte
  NEXT


h2x$  ; ( -- )  uint.16-to-ASCIIz hex via PADx, (implicitly) non-destructive
  ldr x, PADxp  ; ptr to PADx
  ldr x, [x]    ; get PADx
  adr k, N2X    ; ptr to LUT
